...
2024 年中，某家具身公司给我看一段视频。一台双臂机器人在折一件 T 恤。视频开头很顺，机械臂抓住领口，抖了一下展平，开始往中间叠。叠到一半的时候动作突然变得诡异：手臂不是平滑地往内收，而是一抽一抽地，像帕金森晚期，每隔大概 200 毫秒走一小步。T 恤被它折成了一团皱巴巴的东西。

我问他们是不是底层抖动。他们说不是，机械本体跑过 1kHz 的力控测试，没问题。问题在网络这一头。

他们的 VLA 输出的是 7-DoF 动作的离散 token，每个关节 256 个 bin，模型 5Hz 出一个动作 chunk，下游一个简单的 zero-order hold 把它撑到电机的控制频率。**离散 5Hz 的动作命令撞上 1kHz 的伺服回路，结果就是看起来在抽搐**。 token 之间相邻 bin 的实际关节角差了 1.4 度，那一小步在 200 毫秒内被电机硬拉过去，然后 hold 住，然后下一个 token 来了再硬拉一次。
